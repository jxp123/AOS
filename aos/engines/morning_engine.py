...
def morning_briefing():
    ...
    for c in repo.list_apiary_entities(True):
        ...
        nuc_score = nuc_expansion_score(c, latest.get(c['code']))
        rows.append({'code':c['code'],'risk':risk['risk'],'score':risk['score'],'reason':risk['reason'],
                     'priority':dec['priority'],'recommendation':dec['recommendation'],
                     'nuc_expansion':nuc_score.get('recommendation',''), 'confidence':dec['confidence']})
