...
def commit_all_pending():
    ...
    if summary['status']=='FAIL': return {'status':'FAILED','message':'Commit blocked by critical validation issues','backup':None}
    backup=backup_database(); repo=Repository()
    with get_session() as s:
        for p in s.query(PendingCommit).filter(PendingCommit.status=='Pending').all(): p.status='Committed'
        s.commit()
