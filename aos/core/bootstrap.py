...
def seed_data():
    with get_session() as session:
        ...
        if session.query(AuditLog).count() == 0:
            session.add(AuditLog(date=str(datetime.now().replace(microsecond=0)), action='BOOTSTRAP', entity_type='System', entity_code='AOS', details='Seeded v0.7 baseline data.'))
        session.commit()
