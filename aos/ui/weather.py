...
def weather_page():
    ...
    with ui.card().classes('w-full'):
        ...
        def save():
            try:
                repo.create_weather({
                    'date': d.value,
                    'temperature_c': float(temp.value or 0),
                    'wind': wind.value or '',
                    'rain': rain.value or '',
                    'forage_flow': flow.value,
                    'inspection_suitability': suitability.value,
    ...
    def render():
        holder.clear()
        with holder:
            ui.label('Weather / Forage Log').classes('text-h6')
            ui.table(
                columns=[
                    {'name':'date','label':'Date','field':'date'},
                    {'name':'temperature_c','label':'Temp C','field':'temperature_c'},
                    {'name':'wind','label':'Wind','field':'wind'},
                    {'name':'rain','label':'Rain','field':'rain'},
                    {'name':'forage_flow','label':'Forage','field':'forage_flow'},
                    {'name':'inspection_suitability','label':'Inspection','field':'inspection_suitability'},
                    {'name':'notes','label':'Notes','field':'notes'},
                ],
                rows=repo.list_weather(),
                row_key='date',
            ).classes('w-full')
    render()
