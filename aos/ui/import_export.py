...
def import_export_page():
    ...
    with ui.row():
        ui.button('Export AOS to Excel', on_click=do_export); ui.button('Create Manual Backup', on_click=do_backup)
    filename=ui.input('Import filename in imports folder').classes('w-96')
    def do_import():
        try: count=import_colonies_from_excel(IMPORT_DIR / filename.value); ui.notify(f'Processed {count} colonies in import scaffold', type='positive')
