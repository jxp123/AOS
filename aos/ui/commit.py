...
def commit_page():
    ui.label('Commit Queue').classes('text-h5')
    repo=Repository()
    payload=ui.textarea('Stage new pending change').classes('w-full')
    def stage():
        repo.create_pending_commit('Manual Note','System','AOS',payload.value or '')
        ui.notify('Pending change staged', type='positive')
    def validate():
        s=validate_pending_commits(); ui.notify(s['message'], type='warning' if s['status']=='WARN' else 'positive')
    def commit():
        r=commit_all_pending(); ui.notify(r['message'], type='positive' if r['status']=='COMMITTED' else 'negative')
    with ui.row():
