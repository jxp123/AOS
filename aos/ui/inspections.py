...
def inspections_page():
    ...
    repo = Repository()
    log_holder = ui.column().classes('w-full')
    ...
    with ui.card().classes('w-full'):
        ...
        q = ui.checkbox('Queen seen')
        eggs = ui.checkbox('Eggs seen')
        larvae = ui.checkbox('Larvae seen')
        ...
        def save():
            ...
            try:
                repo.create_inspection({
                    ...
                    'queen_seen': bool(q.value),
                    'eggs_seen': bool(eggs.value),
                    'larvae_seen': bool(larvae.value),
                    'queen_cells': int(queen_cells.value or 0),
                    'brood_frames': float(brood.value or 0),
                    'stores_frames': float(stores.value or 0),
                    'bee_coverage_frames': float(bees.value or 0),
