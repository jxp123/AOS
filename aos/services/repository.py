...
class Repository:
    ...
    def create_colony(self, data):
        with get_session() as s:
            if s.query(Colony).filter_by(code=data.get('code')).first():
                raise ValueError(f"Colony already exists: {data.get('code')}")
            s.add(Colony(**data))
            s.commit()
    ...
    def create_queen(self, data):
        with get_session() as s:
            if s.query(Queen).filter_by(code=data.get('code')).first():
                raise ValueError(f"Queen already exists: {data.get('code')}")
            s.add(Queen(**data))
            s.commit()
    ...
    def create_equipment(self, data):
        with get_session() as s:
            if s.query(Equipment).filter_by(code=data.get('code')).first():
                raise ValueError(f"Equipment already exists: {data.get('code')}")
            s.add(Equipment(**data))
            s.commit()
    ...
    def baseline_integrity(self):
        ...
        return {
            'expected_colonies':len(BASELINE_COLONIES),'actual_colonies':len(cc),
            'missing_colonies':[{'code':c[0],'name':c[1],'type':c[2],'equipment':c[3],'status':c[5]} for c in BASELINE_COLONIES if c[0] not in cc],
            'missing_queens':[{'code':q[0],'name':q[1],'line':q[2],'current_colony':q[4]} for q in BASELINE_QUEENS if q[0] not in cq],
            'missing_equipment':[{'code':e[0],'name':e[1],'type':e[2],'location':e[3]} for e in BASELINE_EQUIPMENT if e[0] not in ce],
        }
        ...
        with get_session() as s:
            ...
            for c in BASELINE_COLONIES:
                if not s.query(Colony).filter_by(code=c[0]).first():
                    s.add(Colony(code=c[0],name=c[1],colony_type=c[2],equipment=c[3],objective=c[4],status=c[5],notes=c[6])); added['colonies']+=1
            for q in BASELINE_QUEENS:
                if not s.query(Queen).filter_by(code=q[0]).first():
                    s.add(Queen(code=q[0],name=q[1],line=q[2],source=q[3],current_colony_code=q[4],status=q[5],evidence_status=q[6],notes=q[7])); added['queens']+=1
            for e in BASELINE_EQUIPMENT:
                if not s.query(Equipment).filter_by(code=e[0]).first():
                    s.add(Equipment(code=e[0],name=e[1],type=e[2],current_location=e[3],compatible_with=e[4],status=e[5],notes=e[6])); added['equipment']+=1
            s.add(AuditLog(date=str(datetime.now().replace(microsecond=0)),action='RESTORE_BASELINE',entity_type='System',entity_code='AOS',details=str(added))); s.commit()
        return added
    ...
    def list_weather(self):
        with get_session() as s: return [{'date':w.date,'temperature_c':w.temperature_c,'wind':w.wind,'rain':w.rain,'forage_flow':w.forage_flow,'inspection_suitability':w.inspection_suitability,'notes':w.notes or ''} for w in s.query(WeatherObservation).order_by(WeatherObservation.date.desc()).limit(50).all()]
    def create_weather(self,data):
        with get_session() as s: s.add(WeatherObservation(**data)); s.commit(); self.audit('CREATE','Weather',data.get('date',''),'Weather/forage observation saved')
    def list_inspections(self):
        with get_session() as s: return [{'date':i.date,'colony':i.colony.code if i.colony else '', 'inspection_type':i.inspection_type,'queen_seen':'Yes' if i.queen_seen else 'No','eggs_seen':'Yes' if i.eggs_seen else 'No','queen_cells':i.queen_cells,'brood_frames':i.brood_frames,'stores_frames':i.stores_frames,'bee_coverage_frames':getattr(i,'bee_coverage_frames',0),'temperament':i.temperament,'notes':i.notes or ''} for i in s.query(Inspection).order_by(Inspection.date.desc(),Inspection.id.desc()).limit(300).all()]
    ...
    def create_inspection(self,data):
        with get_session() as s:
            ...
            s.add(Inspection(**data)); s.commit(); self.audit('CREATE','Inspection',colony.code,f"Inspection saved for {data.get('date')}")
    def list_audit(self):
        with get_session() as s: return [{'date':a.date,'action':a.action,'entity_type':a.entity_type,'entity_code':a.entity_code,'details':a.details} for a in s.query(AuditLog).order_by(AuditLog.id.desc()).limit(150).all()]
    ...
    def create_pending_commit(self,event_type,entity_type,entity_code,payload):
        with get_session() as s: s.add(PendingCommit(date=str(datetime.now().replace(microsecond=0)),event_type=event_type,entity_type=entity_type,entity_code=entity_code,payload=payload)); s.commit(); self.audit('STAGE','PendingCommit',entity_code,f'Staged {event_type}')
    def list_pending_commits(self):
        with get_session() as s: return [{'id':p.id,'date':p.date,'event_type':p.event_type,'entity_type':p.entity_type,'entity_code':p.entity_code,'payload':p.payload,'status':p.status,'validation_status':p.validation_status,'validation_message':p.validation_message} for p in s.query(PendingCommit).order_by(PendingCommit.id.desc()).all()]
    @staticmethod
    ...
    @staticmethod
    def _dict_queen(q): return {'id':q.id,'code':q.code,'name':q.name,'line':q.line,'source':q.source,'current_colony':q.current_colony_code,'status':q.status,'evidence':q.evidence_status,'temperament_score':getattr(q,'temperament_score',0),'brood_score':getattr(q,'brood_score',0),'honey_score':getattr(q,'honey_score',0),'notes':q.notes or ''}
    @staticmethod
