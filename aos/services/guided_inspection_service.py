...
class GuidedInspectionService:
    def validate_payload(self, payload):
        ...
        if not payload.get('colony_id'):
            messages.append('Colony/nuc is required.')
            status = 'FAIL'

        if not payload.get('inspection_date'):
            messages.append('Inspection date is required.')
            status = 'FAIL'
        ...
        if brood == 0 and stores == 0 and bees == 0 and not payload.get('notes'):
            messages.append('Very little evidence entered. Add brood, stores, bee coverage or notes.')
            status = 'WARN' if status != 'FAIL' else status

        if queen_cells > 0 and not payload.get('notes'):
            messages.append('Queen cells recorded; add notes describing charged/open/capped and action taken.')
            status = 'WARN' if status != 'FAIL' else status

        if not queen_seen and not eggs_seen and not larvae_seen and brood > 0:
            messages.append('Brood entered but no queen/eggs/larvae evidence selected. Check record.')
            status = 'WARN' if status != 'FAIL' else status

        if stores < 1 and bees >= 5:
            messages.append('Strong bee coverage with low stores. Consider feed/stores review.')
            status = 'WARN' if status != 'FAIL' else status
    ...
    def stage_draft(self, payload):
        with get_session() as s:
            ...
            s.add(AuditLog(
                ...
                action='STAGE',
                entity_type='GuidedInspectionDraft',
                entity_code=colony.code,
                details=f"{validation['status']}: {validation['message']}",
            ))
            s.commit()
            return draft.id, validation, confidence
    ...
    def list_drafts(self):
        with get_session() as s:
            return [{
                ...
                'colony_code': d.colony_code,
                'inspection_date': d.inspection_date,
                'queen_seen': 'Yes' if d.queen_seen else 'No',
                'eggs_seen': 'Yes' if d.eggs_seen else 'No',
                'brood_frames': d.brood_frames,
                'stores_frames': d.stores_frames,
                'bee_coverage_frames': d.bee_coverage_frames,
                'confidence': d.confidence,
                'validation_status': d.validation_status,
                'validation_message': d.validation_message,
                'status': d.status,
                'evidence': d.evidence,
            } for d in s.query(GuidedInspectionDraft).order_by(GuidedInspectionDraft.id.desc()).limit(100).all()]
    ...
    def commit_draft(self, draft_id):
        with get_session() as s:
            ...
            if d.status != 'Staged':
                raise ValueError(f'Draft is not staged: {d.status}')
            if d.validation_status == 'FAIL':
                raise ValueError('Cannot commit a failed draft. Fix and stage again.')

            inspection = Inspection(
                colony_id=d.colony_id,
                date=d.inspection_date,
                inspection_type='Guided',
                queen_seen=d.queen_seen,
                eggs_seen=d.eggs_seen,
                larvae_seen=d.larvae_seen,
                queen_cells=d.queen_cells,
                brood_frames=d.brood_frames,
                stores_frames=d.stores_frames,
                bee_coverage_frames=d.bee_coverage_frames,
            ...
            s.add(AuditLog(
                ...
                action='COMMIT',
                entity_type='GuidedInspectionDraft',
                entity_code=d.colony_code,
                details=f'Committed draft {draft_id} into inspection log.',
            ))
            s.commit()
    ...
    def reject_draft(self, draft_id):
        with get_session() as s:
            ...
            s.add(AuditLog(
                ...
                entity_type='GuidedInspectionDraft',
                entity_code=d.colony_code,
                details=f'Rejected draft {draft_id}.',
            ))
            s.commit()
